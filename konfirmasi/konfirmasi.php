...
<td>:<td colspan="2">
<input type="radio" name="status" id="status"  checked="checked" value="Konfirmasi" <?php if($status=="Konfirmasi"){echo"checked";}?>/>Konfirmasi 
<input type="radio" name="status" id="status" value="Valid" <?php if($status=="Valid"){echo"checked";}?>/>Valid
...
<img src='$YPATH/$bukti_bayar' width='40' height='40' /></a></div>";
	...
	<br> <small>Nominal TF: <u>Rp. ".RP($nominal)."</u></i> <i>Pesan: $pesan</small>			
				...
				<td><div align='center'>
<a href='?mnu=konfirmasi&id=$id_pem_iklan&pro=ubah&kode=$id_konfirmasi'><img src='ypathicon/ub.png' title='ubah'></a>
<a href='?mnu=konfirmasi&id=$id_pem_iklan&pro=hapus&kode=$id_konfirmasi'><img src='ypathicon/ha.png' title='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus  pada data konfirmasi ?..\")'></a></div></td>
				</tr>";
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $id_konfirmasi berhasil disimpan !');document.location.href='?mnu=konfirmasi&id=$id_pem_iklan';</script>";}
		else{echo"<script>alert('Data $id_konfirmasi gagal disimpan...');document.location.href='?mnu=konfirmasi&id=$id_pem_iklan';</script>";}
	}
	else{
	$sql="update `$tbkonfirmasi` set 
	`status`='$status',
	`keterangan`='$keterangan'
	 where `id_konfirmasi`='$id_konfirmasi0'";
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $id_konfirmasi berhasil diubah !');document.location.href='?mnu=konfirmasi&id=$id_pem_iklan';</script>";}
		else{echo"<script>alert('Data $id_konfirmasi gagal diubah...');document.location.href='?mnu=konfirmasi&id=$id_pem_iklan';</script>";}
	}//else simpan
}
?>
...
$id_konfirmasi=$_GET["kode"];
$sql="delete from `$tbkonfirmasi` where `id_konfirmasi`='$id_konfirmasi'";
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_konfirmasi berhasil dihapus !');document.location.href='?mnu=konfirmasi&id=$id_pem_iklan';</script>";}
	else{echo"<script>alert('Data $id_konfirmasi gagal dihapus...');document.location.href='?mnu=konfirmasi&id=$id_pem_iklan';</script>";}
}
?>
