...
<script type="text/javascript">
   tinyMCE.init({
    ...
    <div id="accordion">
  <h3>Request Pemasangan Iklan</h3>
  <div>
...
<td>:<td><input required name="judul" class="form-control" type="text" id="judul" value="<?php echo $judul;?>" size="25" />
</td>
</tr>
<tr>
...
<tr>
<td height="24"><label for="tanggal_mulai">Pengajuan Tanggal Mulai</label>
...
<td>
<td>
<td colspan="2"><input name="Simpan" type="submit" id="Simpan" class="btn btn-primary" value="Ajukan" onclick="tinyMCE.triggerSave(true,true);" />
       ...
       <input name="gambar0" type="hidden" id="gambar0" value="<?php echo $gambar0;?>" />
        ...
        <a href="?mnu=request_iklan"><input name="Batal" type="button" class="btn btn-warning" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
<br />
...
<?php  
  $sqlc="select distinct(`status`) from `$tbpemasangan_iklan` where `id_pelanggan`='".$_SESSION['cid']."' order by `status` asc";
  $jumc=getJum($conn,$sqlc);
	...
	$arrc=getData($conn,$sqlc);
		foreach($arrc as $dc) {						
				$status=$dc["status"];
				  $sql="select * from `$tbpemasangan_iklan` where  `status`='$status' and `id_pelanggan`='".$_SESSION['cid']."' order by `id_pem_iklan` desc";
  $jum=getJum($conn,$sql);
				?>
...
<table class="table table-bordered">
  <tr bgcolor="#cccccc">
   ...
   <th width="20%"><small>Lokasi & Fasilitas</small></td>
    ...
    <th width="20%"><small>Menu</small></td>
  </tr>
<?php  
		if($jum > 0){
//--------------------------------------------------------------------------------------------
...
<img src='$YPATH/$gambar' width='40' height='40' /></a>
				...
				if($status=="Request"){
					echo"<a href='?mnu=gambar&id=$id_pem_iklan'><i><font size='2' color='red'><u>Upload Gambar</u></font></i></a> <br>
					<a href='?mnu=_konfirmasi&id=$id_pem_iklan'><font size='2' color='blue'><u>Bayar</u></font></i></a>
					<a href='?mnu=request_iklan&pro=ubah&kode=$id_pem_iklan'><font size='2' color='green'><u>Ubah Data</u></font></i></a>";}
				else if($status=="Approve"){
					echo"Menunggu Publikasi pada ".$tanggal_mulai."";}
				else if($status=="Mulai"){
					echo"Iklan sedang dipublikasi";}
				else {
					echo"Iklan sudah expired";}
				echo"</div></td>
				</tr>";
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=request_iklan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=request_iklan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=request_iklan'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $judul berhasil disimpan !');document.location.href='?mnu=request_iklan';</script>";}
		else{echo"<script>alert('Data $judul gagal disimpan...');document.location.href='?mnu=request_iklan';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $judul berhasil diubah !');document.location.href='?mnu=request_iklan';</script>";}
		else{echo"<script>alert('Data $judul gagal diubah...');document.location.href='?mnu=request_iklan';</script>";}
	}//else simpan
}
?>
