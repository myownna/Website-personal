...
<td>:<td><?php echo getPelanggan($conn,$id_pelanggan);?> - <?php echo $id_pelanggan;?>
</td>
</tr>
<tr>
<td height="24"><label for="pesan">Pesan</label>
<td>:<td><?php echo $pesan;?>
</td>
</tr>
...
<?php  
  $sqlc="select distinct(`status`) from `$tbinbox` order by `status` asc";
  $jumc=getJum($conn,$sqlc);
	...
	$arrc=getData($conn,$sqlc);
		foreach($arrc as $dc) {						
				$status=$dc["status"];
				$sql="select * from `$tbinbox` where  `status`='$status' order by `id_inbox` desc";
  $jum=getJum($conn,$sql);
				?>
<h3>Data Inbox <?php echo "$status ($jum Inbox)"?>:</h3>
<div>				
...
echo"<tr bgcolor='$color'>
				...
				<td><small>$nama_pelanggan - $id_pelanggan</small></td>
				<td><small> $tanggal $jam  <br> <i>$pesan</i></small></td>
				<td><small>$kategori</small></td>
				<td><small>$status #$keterangan</small></td>
				<td><div align='center'>
<a href='?mnu=inbox&pro=ubah&kode=$id_inbox'><img src='ypathicon/ub.png' title='ubah'></a>
<a href='?mnu=inbox&pro=hapus&kode=$id_inbox&id_pelanggan=$id_pelanggan'><img src='ypathicon/ha.png' title='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $id_pelanggan pada data inbox ?..\")'></a></div></td>
				</tr>";
...
$sql="delete from `$tbinbox` where `id_inbox`='$id_inbox'";
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_pelanggan berhasil dihapus !');document.location.href='?mnu=inbox';</script>";}
	else{echo"<script>alert('Data $id_pelanggan gagal dihapus...');document.location.href='?mnu=inbox';</script>";}
}
?>
